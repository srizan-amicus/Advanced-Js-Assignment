...
async function searchRepositories() {
    ...
    showLoading();
    try {
        const repositories = await apiService.searchRepositories(currentQuery, currentPage, repositoriesPerPage);
        ...
        const repositoryCards = repositories.map(transformRepository);
        setUiState("success");
...
async function goToNextPage(event) {
    ...
    try {
        ...
        console.error("Failed to load next repository page:", error);
        currentPage--;
        setUiState("error");
        showError(error instanceof Error
            ? error.message
...
async function goToPreviousPage(event) {
    ...
    try {
        ...
        console.error("Failed to load previous repository page:", error);
        currentPage++;
        setUiState("error");
        showError(error instanceof Error
            ? error.message
